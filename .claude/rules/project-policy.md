...
- **Zustand** で状態管理。`src/stores/` は全 persist 化済み：個人データ系（`profile-store`・`favorites-store`・`search-conditions-store`・`questionnaire-store`）と、ローカルキャッシュ系（`hospitals-store`・`transport-services-store`・`emergency-rotations-store`、ticket 15 で persist + 24h TTL + `hasHydrated` + `load(force?)` を付与済み）。キャッシュの鮮度判定は `src/lib/cache.ts` の `isFresh`、起動/復帰/手動の最新化は `src/hooks/use-cache-sync.ts`
