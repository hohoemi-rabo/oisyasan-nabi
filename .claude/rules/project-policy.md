...
- **Zustand** で状態管理。`src/stores/` に persist あり 3 つ（`profile-store`・`favorites-store`・`search-conditions-store`）と persist なし 2 つ（`hospitals-store`・`transport-services-store`、ticket 15 で persist 被せる予定）
...
- **メモリ常駐ストアの API**: 永続化なしストアでも `{ data, loadedAt, isLoading, error, load() }` の shape で書く。ticket 15 で `persist` + `loadedAt` ベースの 24h TTL を上から被せやすくするため（`hospitals-store` / `transport-services-store` 参照）。
